...
1. Client sends a GET request ot /foos with two headers `Accept: application/json` and `Content-type: application/xml`. To tell server that the request body is in xml and we want the server to return json.
...
3. When spring controller returns a response. With @ResponseBody and `Accept` of client, the return value/object will be converted from specified java entity to json by json converter.
...
**`Content-Type` header specified by the Client will be used to determine the appropriate converter for it**. So `Content-Type: application/json` here to choose a json converter which will convert json content in request body to User object in method argument.
